...
<template>
  ...
  <TresCanvas
    v-bind="gl"
    :tone-mapping-exposure="exposure.value.root"
  >
    <TresPerspectiveCamera :position="[0, 100, 2000]" />
    <Sky
      :elevation="elevation.value.root"
      :azimuth="azimuth.value.root"
      :mie-coefficient="mieCoefficient.value.root"
      :mie-directional-g="mieDirectionalG.value.root"
      :rayleigh="rayleigh.value.root"
      :turbidity="turbidity.value.root"
      :distance="distance.value.root"
    />
    <OrbitControls 
      :enable-pan="false"
      :enable-zoom="false"
