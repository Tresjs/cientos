...
import CatmullRomCurve3 from './component.vue'
import OrbitControls from '../OrbitControls/component.vue'
import { TresCanvas, useRenderLoop } from '@tresjs/core'
import { ref } from 'vue'

const NUM_POINTS = 10
const points = ref<[number, number, number][]>(Array.from({ length: NUM_POINTS }).fill(0).map((_, i) => [i * 0.1, 0, 0]))
const colors = ref<[number, number, number][]>([
  [1.0, 0.0, 0.0],
  [0.9, 0.1, 0.0],
  [0.8, 0.2, 0.0],
  [0.7, 0.3, 0.0],
