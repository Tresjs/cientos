...
const styles = computed(() => {
  if (transform.value) {
    ...
    return {
      ...
      ...(fullscreen.value && {
        top: `-${(sizes.height.value) / 2}px`,
        left: `-${(sizes.width.value) / 2}px`,
        width: `${sizes.width.value}px`,
        height: `${sizes.height.value}px`,
      }),
...
watch(
  ...
  ([group, renderer]): void => {
    if (group && renderer && camera.activeCamera.value) {
      isMeshSizeSet.value = false

      scene.value?.updateMatrixWorld()

      if (transform.value) {
        el.value.style.cssText = 'position:absolute;top:0;left:0;pointer-events:none;overflow:hidden;transform-style:preserve-3d;'
      }
      else {
        const vector = calculatePosition.value(group, camera.activeCamera.value as TresCamera, {
          width: sizes.width.value,
          height: sizes.height.value,
        })

        el.value.style.cssText = `position:absolute;top:0;left:0;transform:translate3d(${vector[0]}px,${vector[1]}px,0);transform-origin:0 0;will-change: transform`
      ...
      if (portal?.value) {
        ...
        mountTarget = canvas?.parentNode as HTMLElement
      }
...
onBeforeRender(({ invalidate }) => {
  ...
  if (groupRef.value && camera.activeCamera.value && renderer.instance) {
    ...
    if (
      ...
      if (raytraceTarget) {
        const isCurrentlyVisible = isObjectVisible(
          groupRef.value,
